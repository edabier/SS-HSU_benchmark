...
def run_one_xp(i_dataset, upsampler, unmixer, n_features, i_xp, dataset, mse_tensor, sad_tensor, Y_init, A_init, E_init, wavelengths, H, dev):
    ...
    for i in range(n_train): 
        ...
        if unmixer == 1:
            model = unmx.UnmixingFromFeatures(D=D, alpha=alpha, B=B, c=c, H=new_H, upsampler=upsampler, n_features=n_features)
        elif unmixer == 2:
            model = unmx.UnmixingFromFeatures2(D=D, alpha=alpha, B=B, c=c, H=new_H, upsampler=upsampler, n_features=n_features)
...
def main(args, dev):
    ...
    for i_dataset, dataset in enumerate(datasets):
        ...
        for i_xp in range(n_xp):
            ...
            mse_tensor, sad_tensor = run_one_xp(i_dataset, upsampler, unmixer, n_features, i_xp, dataset, mse_tensor, sad_tensor, Y_init, A_init, E_init, wavelengths, H, dev)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--upsampler", default="FiLM", type=str)
    parser.add_argument("--unmixer", default=2, type=int)
    ...
    run = wandb.init(project=f"DOFA_{args.upsampler}_unmixer{args.unmixer}")
