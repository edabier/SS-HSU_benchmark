...
def get_position_code(col, in_window=3, out_window=7):
    
    n_r = math.ceil(col / in_window)
    n_c = math.ceil(col / in_window)
    position_matrix = np.zeros((col, col))
    ...
    half = (out_window - 1) // 2
    position_fea = np.zeros((col + out_window, col + out_window))

    position_fea[half:half + col, half:half + col] = position_matrix
    position_dim = n_r * n_c
    position_code = np.zeros((col, col, n_r * n_c))
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=95, patch_size=19, in_chans=156, outer_dim=200, inner_dim=24, inner_stride=1):
        super().__init__()
...
class TNT(nn.Module):
    ...
    """
    def __init__(self, *, img_size, patch_size, in_chans, outer_dim, inner_dim,
                 depth=12, outer_num_heads=12, inner_num_heads=4, mlp_ratio=4.,
                 qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0.2, norm_layer=nn.LayerNorm, inner_stride=4, se=0):
        super().__init__()
