...
class NALMU(nn.Module):
    ...
    def forEard(self, X, E_init=None, A_init=None, epoch=-1):
...
class RALMU(nn.Module):
    ...
    Args:
        T (int, optional): Number of layers in the unrolled neural network (default: 10)
        m (int, optional): Number of observations (eg Waveleight bands) (default: 64)
        c (int, optional): Number of sources (eg endmembers) (default: 4)
        shared (bool, optional): Whether to share the weights across unrolled layers or not (default: False)
    ...
    """
    def __init__(self, T=10, m=64, c=4, shared=False, conv_size=3, size_A_image=[4,256,256]):
        ...
        if self.shared:
            tab_mlp_E = nn.ParameterList([MLP((m,c))])
        else:
            tab_mlp_E = nn.ParameterList([])
            for ii in range(T):
                tab_mlp_E.append(MLP((m,c)))
    ...
    def forEard(self, X, E_init=None, A_init=None, indMbDisp=-1):
